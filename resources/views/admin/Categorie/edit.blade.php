        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-2">
                ...
                <div class="mt-6 p-2 bg-slate-100">
                    <h2 class="text-2xl font-semibold">Category Permissions</h2>
                    <!-- Update the following code to match your new logic for category permissions -->
                    <div class="flex space-x-2 mt-4 p-2">
                        @if ($category->permissions)
                            @foreach ($category->permissions as $category_permission)
                                <form class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-md" method="POST"
                                    action="{{ route('admin.categories.permissions.revoke', [$category->id, $category_permission->id]) }}"
                                    onsubmit="return confirm('Are you sure?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit">{{ $category_permission->name }}</button>
                                </form>
                            @endforeach
                        @endif
                    </div>
